...
Point toCalibrate(0,0); //centro do circulo
string calibrationFile = "calibration.txt";
string templateFile = "template.png";


//Point é uma estrutura OpenCv para a definição de um ponto no plano.
...
void updateSize(){
    int ABx = abs(calibrationFirstArray[0].x - calibrationFirstArray[1].x); //(-10 - (-10))=0
    int CDx =  abs(calibrationFirstArray[2].x - calibrationFirstArray[3].x);
    windowResultWidth = ABx < CDx ? CDx : ABx; //False -> ABx = 0
    ...
    windowResultHeight = ACy < BDy ? BDy : ACy; //False -> ACy = 0
    calibrationSecondArray[1]= Point(windowResultWidth, 0);  //Todas as coordenadas estarão no ponto(0,0)
...
void toggleDrag(int status, void* data){ //Volta os valores inicial
    currentButton = -1;
    dragMouse = true;
}
...
//OpenCv provê um tipo basico de estrutura para armazenar imagem: A classe Mat, as imagens são
